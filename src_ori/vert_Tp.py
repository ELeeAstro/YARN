...
def isothermal(p_lay: jnp.ndarray, params: Dict[str, jnp.ndarray]):
    nlay = jnp.size(p_lay)
    T_iso = jnp.asarray(params["T_iso"])
    return jnp.full((nlay,), T_iso)


def Milne(p_lay: jnp.ndarray, params: Dict[str, jnp.ndarray]):
    log_g = jnp.asarray(params["log_g"])
    T_int = jnp.asarray(params["T_int"])
    k_ir = jnp.asarray(params["k_ir"])
    g = 10.0 ** log_g
    tau_ir = k_ir / g * p_lay
    return (0.75 * T_int**4 * (hopf_function(tau_ir) + tau_ir)) ** 0.25


def Guillot(p_lay: jnp.ndarray, params: Dict[str, jnp.ndarray]):
    T_int = jnp.asarray(params["T_int"])
    ...
    return (milne + guillot) ** 0.25


def Line(p_lay: jnp.ndarray, params: Dict[str, jnp.ndarray]):
    T_int = jnp.asarray(params["T_int"])
    k_ir = jnp.asarray(params["k_ir"])
    ...
    return (milne + line) ** 0.25


def Barstow(p_lay: jnp.ndarray, params: Dict[str, jnp.ndarray]):
    T_iso = jnp.asarray(params["T_iso"])
    kappa = 2.0 / 7.0
    p1 = 0.1 * bar
    p2 = 1.0 * bar
    p_for_adiabat = jnp.maximum(p_lay, p1)
    T_adiabat = T_iso * (p_for_adiabat / p1) ** kappa
    T_deep = T_iso * (p2 / p1) ** kappa
    return jnp.where(p_lay <= p1, T_iso, jnp.where(p_lay <= p2, T_adiabat, T_deep))
