...
Source layout:
  - ../../src_ori/vert_struct.py      OR
  - ../../src_ori/phys_vert_struct.py

with functions like:
    isothermal(nlay, p_lev, params_dict)
    Milne(nlay, p_lev, params_dict)
    Guillot(nlay, p_lev, params_dict)
...
def _choose_vert_struct_model(cfg) -> str:
    ...
    Uses cfg.physics.vert_stuct (note the spelling, as in your YAML).
    Falls back to 'vert_struct' if present.
    """
    phys = getattr(cfg, "physics", None)
    if phys is None:
    ...
    if not name:
        # In case you later rename it
        name = getattr(phys, "vert_struct", None)
    if not name:
        raise ValueError(
            "cfg.physics.vert_stuct (or vert_struct) missing; "
...
def plot_Tp_band(
    ...
    # Try vert_struct.py first, then phys_vert_struct.py as a fallback
    vs_mod = None
    try:
        import vert_struct as vs_mod  # type: ignore
    except ImportError:
        try:
            import phys_vert_struct as vs_mod  # type: ignore
        except ImportError as e:
            raise ImportError(
                f"Could not import vert_struct or phys_vert_struct from {src_root}."
            ) from e
    ...
    T_samples = np.empty((M, n_lay), dtype=float)
    ...
    for k, ii in enumerate(idx):
        ...
        # vert_struct_fn expects: (nlay, p_lev, params_dict)
        # p_lev is an array of interface pressures (bar)
        T_lay = vert_struct_fn(p_lev, pars)
        T_samples[k, :] = np.asarray(T_lay, dtype=float)

    # Pointwise quantiles in T at each layer
    T_p02_5 = np.quantile(T_samples, 0.025, axis=0)
    T_p50   = np.quantile(T_samples, 0.50,  axis=0)
    T_p97_5 = np.quantile(T_samples, 0.975, axis=0)
    ...
    ax.plot(T_p50, p_lay/1e6, lw=2, label="Median T(p)", color=palette[1])
